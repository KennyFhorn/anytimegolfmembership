...
2. In the SQL Editor, run the migrations in order: `supabase/migrations/0001_init.sql`,
   ...
   `0007_league_night_game_type.sql` — these create all tables, RLS policies, the auth triggers, a
   ...
   before 0004 added those columns, and the `game_type` column league nights use for their round
   format (see `lib/game-types.ts`).
